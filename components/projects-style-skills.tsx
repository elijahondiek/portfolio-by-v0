...
export default function SidebarNavigationLayout({
  ...
  const shareItem = (item: SidebarItem) => {
    if (navigator.share) {
      navigator.share({
        ...
        url: (contentType === "projects" && item.liveUrl) || window.location.href,
      })
    } else {
      // Fallback to clipboard
      navigator.clipboard.writeText((contentType === "projects" && item.liveUrl) || window.location.href)
    }
  }

  // Find related items based on category or skills/technologies
  const getRelatedItems = () => {
    if (!items || items.length === 0) return []
    if (!selectedItem.technologies && !selectedItem.skills) return []

    const relatedItems = items
      .filter((item) => {
        if (item.id === selectedItem.id) return false
        if (contentType === "projects") {
          return item.technologies?.some((tech) => selectedItem.technologies?.includes(tech)) || false
        } else {
          return item.skills?.some((skill) => selectedItem.skills?.includes(skill)) || false
        }
      })
      .slice(0, 3)
    return relatedItems
  ...
  return (
    <section className="py-12 px-6 relative" ref={containerRef}>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
              <h4 className="text-sm font-medium mb-2 text-[#b4bcd0]">
                {filteredItems.length} {contentType === "projects" ? "Project" : "Skill"}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <p className="text-[#b4bcd0] text-xs truncate max-w-[180px]">{item.tagline}</p>
                        </div>
                      </div>
                      <div
                        ...
                        style={{
                          backgroundColor:
                            contentType === "projects" && item.status ? getStatusColor(item.status) : "#00ff88",
                        }}
                      ></div>
                    </div>
                  </button>
            ...
            <div ref={contentRef} className="h-full overflow-y-auto pr-2 custom-scrollbar">
              ...
              <div className="mb-8 pb-6 border-b border-white/10">
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-[#b4bcd0] text-lg">{selectedItem.tagline}</p>
                  </div>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {(contentType === "projects" ? selectedItem.technologies || [] : selectedItem.skills || []).map(
                    (item, index) => (
                      <span
                        key={index}
              ...
              <div className="space-y-8">
                ...
                <div>
                  ...
                  <p className="text-[#b4bcd0] leading-relaxed">{selectedItem.longDescription}</p>
                </div>
                ...
                <div>
                  ...
                  <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {(contentType === "projects" ? selectedItem.features || [] : selectedItem.skills || []).map(
                      (item, index) => (
                ...
                {contentType === "projects" && selectedItem.metrics && selectedItem.metrics.length > 0 && (
                  <div>
                    <h3 className="text-xl font-semibold mb-4 flex items-center">
                  ...
                  <div className="flex items-center justify-between glass-card p-4 rounded-lg">
                    <div className="flex items-center">
                      ...
                      <span className="text-[#b4bcd0]">
                        {contentType === "projects" ? "Completed on" : "Last updated"}
                      </span>
                    </div>
                ...
                <div>
                  <h3 className="text-xl font-semibold mb-4">
                    Related {contentType === "projects" ? "Projects" : "Skill Categories"}
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {getRelatedItems().map((item) => (
                      ...
                      >
                        ...
                        <p className="text-[#b4bcd0] text-xs line-clamp-2">{item.tagline}</p>
                    ...
                    {getRelatedItems().length === 0 && (
                      <div className="col-span-3 glass-card p-6 text-center">
                        <p className="text-[#b4bcd0]">No related {contentType} found</p>
                      </div>
                    )}
                  </div>
                </div>
...
const skillsCategories = ["All", "Languages", "Frameworks", "DevOps", "Cloud", "Databases", "Testing", "CMS"]
