...
import { ArrowRight, ChevronDown, ExternalLink, Code, Sparkles } from "lucide-react"
...
export default function Hero() {
  ...
  const techIcons = [
    ...
    { name: "JavaScript", symbol: "JS", x: 85, y: 15, size: 20, delay: 0.5 },
    { name: "TypeScript", symbol: "TS", x: 75, y: 85, size: 22, delay: 1 },
    { name: "Python", symbol: "🐍", x: 10, y: 70, size: 26, delay: 1.5 },
    { name: "AWS", symbol: "☁️", x: 60, y: 40, size: 28, delay: 2 },
    { name: "Node.js", symbol: "🟢", x: 30, y: 90, size: 18, delay: 2.5 },
    { name: "GraphQL", symbol: "◢", x: 90, y: 60, size: 20, delay: 3 },
    { name: "Docker", symbol: "🐳", x: 20, y: 40, size: 22, delay: 3.5 },
    { name: "Git", symbol: "⎇", x: 50, y: 75, size: 24, delay: 4 },
  ]
  ...
  return (
    <motion.section
      id="home"
      className="relative min-h-screen w-full overflow-hidden bg-gradient-to-b from-[#0a0f1c] to-[#111827] flex items-center justify-center"
      ref={containerRef}
      style={{ opacity, y }}
    >
      ...
      <div className="container mx-auto px-6 z-10 flex flex-col lg:flex-row items-center justify-between gap-12 pt-20 pb-16">
        ...
        >
          {/* Animated badge */}
          <motion.div
            className="inline-flex items-center px-3 py-1.5 rounded-full bg-gradient-to-r from-blue-500/10 to-blue-600/10 border border-blue-500/20 backdrop-blur-sm"
          ...
          <div className="overflow-hidden">
            <motion.h1 className="text-5xl md:text-7xl font-bold tracking-tight text-white" variants={itemVariants}>
              Elijah Ondiek
            </motion.h1>
          </div>
          ...
          <motion.div variants={itemVariants} className="space-y-1">
            ...
            <p className="text-lg text-slate-400">Contracting with Safaricom PLC</p>
          </motion.div>

          {/* Mission statement with word-by-word animation */}
          <div className="pt-4 pb-8">
            <motion.p className="text-xl md:text-2xl text-white/90 font-light leading-relaxed flex flex-wrap gap-x-2">
          ...
          <motion.div className="flex flex-wrap gap-4 pt-4" variants={itemVariants}>
            {/* Primary CTA */}
            <motion.button
              className="group relative overflow-hidden rounded-xl bg-blue-600 px-6 py-4 text-white shadow-lg shadow-blue-500/25 transition-all duration-300"
              whileHover={{
                scale: 1.02,
            ...
            {/* Secondary CTA */}
            <motion.button
              className="group relative rounded-xl bg-transparent border-2 border-blue-500/50 px-6 py-4 text-blue-400 transition-all duration-300"
              whileHover={{
                scale: 1.02,
          ...
          <motion.div className="flex flex-wrap gap-3 pt-8" variants={itemVariants}>
            ...
            {["React", "TypeScript", "Node.js", "Python", "AWS"].map((tech, i) => (
              <motion.span
                key={i}
                className="px-2.5 py-1 text-xs rounded-md bg-white/5 text-slate-300 border border-white/10"
            ...
            <motion.span
              className="px-2.5 py-1 text-xs rounded-md bg-white/5 text-slate-300 border border-white/10 cursor-pointer"
              whileHover={{ scale: 1.05, backgroundColor: "rgba(255, 255, 255, 0.1)" }}
            >
              +12 more
            </motion.span>
        ...
        <div className="w-full lg:w-5/12 relative">
          ...
          <div className="relative h-[400px] md:h-[500px] w-full perspective-1000">
            {/* Main profile card */}
            <motion.div
              className="absolute inset-0 rounded-2xl overflow-hidden bg-gradient-to-br from-slate-900 to-slate-800 border border-white/10 shadow-xl"
                ...
                <div className="flex-1 overflow-hidden">
                  <div className="space-y-2 font-mono text-sm">
                    ...
                    <div className="flex pl-16">
                      ...
                      <span className="text-amber-300 mx-2">'elijah@example.com'</span>
                    </div>
                    <div className="flex pl-8">
          ...
          {/* Improved Floating achievements with better visibility */}
          <div className="absolute -left-20 -right-20 -top-20 -bottom-20 pointer-events-none">
            {achievements.map((achievement, index) => {
              // Calculate position based on index
              let positionClass = ""
              if (index === 0) positionClass = "top-[10%] left-[0%]"
              if (index === 1) positionClass = "top-[15%] right-[5%]"
              if (index === 2) positionClass = "bottom-[20%] left-[10%]"
              if (index === 3) positionClass = "bottom-[10%] right-[0%]"

              // Hide some achievements on mobile
              ...
              return (
                <motion.div
                  key={index}
                  className={`absolute ${positionClass} ${visibilityClass} z-20`}
                ...
                >
                  {/* Improved achievement card with better visibility */}
                  <motion.div
                    className={`${achievement.bgColor} backdrop-blur-md border-2 ${achievement.borderColor} rounded-xl p-4 shadow-lg shadow-black/30`}
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      ...
                      <div
                        className={`flex items-center justify-center w-10 h-10 rounded-full ${achievement.iconBg} text-white text-xl`}
                      >
                        {achievement.icon}
                      </div>
                      ...
                      <div>
                        ...
                        <div className="text-sm font-medium text-white/80">{achievement.label}</div>
                      </div>
                    </div>
                  </motion.div>
                </motion.div>
      ...
      <div className="absolute top-0 left-0 w-full h-32 bg-gradient-to-b from-[#0a0f1c] to-transparent pointer-events-none"></div>
    </motion.section>
  )
}
